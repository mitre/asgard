...
$0 generates a named volume of combined_heimdall_secrets which contains random
...
docker inspect combined_heimdall_secrets >/dev/null 2>/dev/null \
	&& [[ $1 != --overwrite ]] \
...
of new keys. This will break any existing Heimdall database containing users." 1>&2 \
	&& exit 0
docker volume create combined_heimdall_secrets
docker run -v combined_heimdall_secrets:/srv/secrets --name heim_helper busybox true
