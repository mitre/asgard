...
$0 generates a named volume of heimdall_secrets which contains random
...
docker inspect heimdall_secrets >/dev/null 2>/dev/null \
	&& [[ $1 != --overwrite ]] \
...
of new keys. This will break any existing Heimdall database containing users." 1>&2 \
	&& exit 0
docker volume create --name=heimdall_secrets
docker run -v heimdall_secrets:/srv/secrets --name heim_helper busybox true
